<template>
	<view class="box">
		...
		<view class="basicinformation">
			...
			<view class="text">
				<view class="span">联系人：</view> {{data.LinkManTel}}
			</view>
			<view class="text">
		...
		<view class="basicinformation"  v-if="MSSODtoState"  >
			...
			<view class="text" style="background-color: #dfdfe5;border-radius: 5px;margin-bottom: 5px;" v-for="(item,indexsss) in MSSODto.GoosDtos" :key='indexsss'>
				...
				<view v-for="(items,indexs) in item.EquPhotos" :key="indexs" class="EquPhotos">
					...
					<view class="pushimg">
						<view @tap="addimgleright(items,1)" class="left right">
							<image @click="previewImg(items,0)" class="images" v-if="items.Before" src="../../../../static/img/image.png"
							 mode=""></image>
							<anuploadImgmaintenance v-if="!items.Before"></anuploadImgmaintenance>
						</view>
						<view @tap="addimgleright(items,2)" class="left ">
							<image @click="previewImg(items,1)" class="images" v-if="items.After" src="../../../../static/img/image.png"
							 mode=""></image>
							<anuploadImgmaintenance v-if="!items.After"></anuploadImgmaintenance>
						</view>
		...
		<u-button v-if="this.state == 1 ||this.state == 8 || this.state == 9" :disabled="GOstatus" class="button" @tap="zhi">指
			派</u-button>
		<!-- 接受 ？？ 拒绝-->
...
<script>
	...
	import anuploadImgmaintenance from '@/components/an-uploadImg/an-uploadImg_maintenance.vue' //上传图片
	import hxpreviewimg from '@/components/hx-preview-img/hx-preview-img.vue'
	export default {
		components: {
			uniPopup,
			anuploadImgmaintenance,
			hxpreviewimg
		},
		data() {
			return {
