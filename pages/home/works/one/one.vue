<template>
 ...
 -->
		...
		<view class="thisphotos" v-if="this.RepairFaultImages.length != 0 ">
			<image v-if="item.Type == 'picture'" v-for="(item,index) in this.RepairFaultImages" :key="index" :src="item.Path"
			 mode="" @tap='yulan(index)'></image>
		...
		<button v-if="this.codeurlids == 3 " class="bulone button" @tap="good(47)">出发</button>
		<button v-if="this.codeurlids == 4 " class="bulone button" @tap="good(48)">到达</button>
...
<script>
	export default {
		...
		methods: {
			...
			Finishcreating: function() { //工单完成并创建订单
