<template>
	<view class="box">
		...
		<u-picker v-model="show" mode="selector" :range="selectorObj" @confirm='confirmS' range-key="cateName" :default-selector="[4]"></u-picker>


		<view style="width: 100%;height: 30px;"></view>

		<u-card v-for="(item,index) in Data" :key="index + 'fei' " :title="item.CustomerFullName" :head-style='headStyle'
		 :body-style='bodyStyle' box-shadow='2px 3px 4px #c1c1c1' :sub-title=" '超 ' + String(item.Overtime)"
		 :sub-title-color="item.Overtime > 14 ? 'red' : '#ffaa00'" class='ZTstate'>
			<view class="" slot="body">
				<view class="textCode">
					{{item.Area + ' - ' + item.TaskCode}}
					<span style='float: right;'>{{ '预 ' + ( item.EstimateDate == null ? '' : item.EstimateDate.split('T')[0]) }}</span>
				</view>
			</view>
		</u-card>
