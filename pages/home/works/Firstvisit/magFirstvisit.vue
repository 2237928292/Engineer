	...
	<view class="box">
		...
		<view class="basicinformation" style="padding-left: 10px;"  v-if="Dto.State != 1" >
			<u-form :model="form" ref="uForm"  >
				<u-form-item label-width='200rpx'  label="售后产值(万)"><u-input type='number' auto-height placeholder="请输入金额" v-model="form.OutputValue" /></u-form-item>
				<u-form-item label-width='200rpx'  label="隶属集团"><u-input type='textarea' maxlength="99999" auto-height v-model="form.AffiliatedGroup" /></u-form-item>
				<u-form-item label-width='200rpx'  label="审批流程"><u-input type='textarea' maxlength="99999" auto-height v-model="form.ApprovalProcess" /></u-form-item>
				<u-form-item label-width='200rpx'  label="财务预算"><u-input type='textarea' maxlength="99999" auto-height v-model="form.FinancialBudget" /></u-form-item>
				<u-form-item label-width='200rpx'  label="建店年份"><u-input type='number' auto-height placeholder="请输入建店年份" v-model="form.BuildYear" /></u-form-item>
				
				<u-form-item label-width='200rpx'  label="其它信息"><u-input type='textarea' maxlength="99999" auto-height v-model="form.OtherInfo" /></u-form-item>
			</u-form>
		</view>
		<u-button v-if="Dto.State == 2" size='mini' style='float: right;margin-right: 10px;' @tap='CreateProuctionList()' type="success">设备分类</u-button>
		<u-button v-if="Dto.State == 2" size='mini' style='float: right;margin-right: 10px;' @tap='CreateProuction()' type="success">添加设备</u-button>
		
		<!-- 备注 -->
		<view class="cu-form-group" v-if="Dto.State != 1" >
			<textarea id="Content" style="width: 100%;height: 300px;" maxlength='-1'  v-model="indexEquOverview" placeholder="请输入设备概述" />
		</view>
		
		
		<u-button v-if="Dto.State == 2" size='mini' style='float: right;margin-right: 10px;' @tap='CreateCusLinkMan()' type="success">添加联系人</u-button>
	...
	export default {
		...
		methods:{
			...
			UpdateCusVisitState:function(index){
				...
				this.$http(obj).then((res) => {
					var obj = {
						url: this.$store.state.url + 'CRM/UpdateCusVisitState',
						method: 'POST',
						header: this.$store.state.token,
						data:{
							Id:this.$store.state.FirstvisitIdindexId,
					...
					this.$http(obj).then((res) => {
						this.$store.state.indexcurrent = index
						if(index == 3){
							uni.navigateTo({
								url: '/pages/home/works/Firstvisit/Firstvisit',
							})
