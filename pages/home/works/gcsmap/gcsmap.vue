...
<script>
	export default {
		...
		created() {
			...
			console.log(this.wgs84togcj02(
				'116.55078','39.801903'
			)[0],this.wgs84togcj02(
				'116.55078','39.801903'
			)[1])
			this.bd_encrypt(116.55078,39.801903)
		...
		methods: {
			...
			SelectTrajectory: function(Id) {
				...
				this.$httpnone(obj).then((res) => {
					...
					for (var i = 0; i < List.length; i++) {
						...
						ListIP.push({
							latitude: this.wgs84togcj02(
								List[i].Describe,
								List[i].Key,
							)[1],
							longitude: this.wgs84togcj02(
								List[i].Describe,
								List[i].Key,
							)[0]
						})
			...
			initmapsGCS: function() {
				...
				this.$httpnone(obj).then((res) => {
						...
						for (let index = 0; index < EngineerPositioning.length; index++) {
							...
							EngineerPositioning[index].Longitude = this.wgs84togcj02(
								EngineerPositioning[index].Longitude,
								EngineerPositioning[index].Latitude
							)[0];
							EngineerPositioning[index].Latitude = this.wgs84togcj02(
								EngineerPositioning[index].Longitude,
								EngineerPositioning[index].Latitude
							)[1];
							// 以下是将标志状态转为中文便于展示
			...
			test: function() {
				...
				// that.initmapsGCS()
				console.log(that.$store.state.http)
				that.polylines = [{
					points: [],
					arrowLine: true,
					dottedLine: true,
					arrowIconPath: '../../../../static/icon/icon/video.png',
					width: 10,
					color: "#090"
				}];
				//地图标记点静态数据赋值;
